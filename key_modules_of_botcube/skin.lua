...
local PRE = "botanical_cube_building_" --你自己的mod前缀
...
local function InitSkin(ent, skin, skin_id)
    skin_id = skin_id or PREFAB_SKINS_IDS[ent.prefab] and PREFAB_SKINS_IDS[ent.prefab][skin]
...
getmetatable(TheSim).__index["ReskinEntity"] = function(self, guid, skinname, linked_skinname, skin_id, userid, ...)
    ...
    if inst and skinname ~= linked_skinname then
        ...
        InitSkin(inst, linked_skinname)
    end

    return OldReskinEntity(self, guid, skinname, linked_skinname, skin_id, userid, ...)
end
