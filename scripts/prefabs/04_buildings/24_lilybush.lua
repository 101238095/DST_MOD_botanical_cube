local BERRY_TYPES = { "berries", "berriesmore", "berriesmost" }--浆果，更多浆果，最多浆果
local function setberries(inst, pct)
    ...
    for i, berry_type in ipairs(BERRY_TYPES) do
        if berry_type == berries then
            ...
            inst.AnimState:PlayAnimation("grow",true)
        end
    end
end
...
local function makebarrenfn(inst)--, wasempty)
    if not POPULATING and (inst:HasTag("withered") or inst.AnimState:IsCurrentAnimation("idle",true)) then
...
--[[local function spawnperd(inst)--生成火鸡
...
local function createbush(name, inspectname, berryname, master_postinit)
    ...
    local function fn()
        ...
        inst.scrapbook_specialinfo = "NEEDFERTILIZER"--需要施肥
        ...
        inst:AddComponent("pickable")--可摘取
        ...
        inst:AddComponent("witherable")--枯萎组件

        inst:AddTag("flower")
        
        MakeLargeBurnable(inst)
        MakeMediumPropagator(inst)--传播者
       ...
       --[[ if IsSpecialEventActive(SPECIAL_EVENTS.YOTG) then
        ...
        if is_quagmire then
            event_server_data("quagmire", "prefabs/botanical_cube_building_lilybush").master_postinit(inst)
        end
...
local function petals_postinit(inst)--多汁浆果
    inst.components.pickable:SetUp("petals", TUNING.BERRY_JUICY_REGROW_TIME, 3)
...
return --createbush("berrybush", "berrybush", "berries", normal_postinit),
    ...
    createbush("botanical_cube_building_lilybush", "botanical_cube_building_lilybush", "petals", petals_postinit),
    --名字，动画，摘取物品（应该吧）
    
    MakePlacer("botanical_cube_building_lilybush_placer", "botanical_cube_building_lilybush", "botanical_cube_building_lilybush", "idle")