...
local function FixUpFenceOrientation(inst, deployedrotation)
    ...
    while neighbor ~= nil do
        ...
        if (neighbor.isdoor or neighbor.prefab == "botanical_cube_building_archway") and (this_e % (ROT_SIDES/2) == neighbor_e % (ROT_SIDES/2)) then
    ...
    if neighbor ~= nil then
        ...
        if (neighbor.isdoor or neighbor.prefab == "botanical_cube_building_archway") and (this_e + ROT_SIDES/2) % ROT_SIDES == neighbor_e then
            rot = rot + 180
...
local function MakeWall(name, anims, isdoor, klaussackkeyid, data)
    ...
    local function fn()
        ...
        inst:AddTag("wall")
        inst:AddTag("botanical_cube_building_archway")
